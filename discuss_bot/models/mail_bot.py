...
class MailBot(models.AbstractModel):
    ...
    def _apply_logic(self, record, values, command=None):
        ...
        The logic will only be applied if discussbot is in a chat with a user or
        if someone pinged discussbot.

         :param record: the mail_thread (or mail_channel) where the user
            message was posted/discussbot will answer.
        ...
        if len(record) != 1 or values.get("author_id") == discussbot_id:
            return
        if self._is_bot_pinged(values) or self._is_bot_in_private_channel(record):
            ...
            if answer:
                ...
                record.with_context(mail_create_nosubscribe=True).sudo().message_post(body=answer, author_id=discussbot_id, message_type=message_type, subtype_id=subtype_id)

    def _get_answer(self, record, body, values, command=False):
        # onboarding
        discussbot_state = self.env.user.discussbot_state
        if self._is_bot_in_private_channel(record):
            # main flow
            if discussbot_state == 'onboarding_emoji' and self._body_contains_emoji(body):
                self.env.user.discussbot_state = "onboarding_command"
                self.env.user.discussbot_failed = False
                return _("Great! 👍<br/>To access special commands, <b>start your sentence with</b> <span class=\"o_discussbot_command\">/</span>. Try getting help.")
            elif discussbot_state == 'onboarding_command' and command == 'help':
                self.env.user.discussbot_state = "onboarding_ping"
                self.env.user.discussbot_failed = False
                return _("Wow you are a natural!<br/>Ping someone with @username to grab their attention. <b>Try to ping me using</b> <span class=\"o_discussbot_command\">@OdooBot</span> in a sentence.")
            elif discussbot_state == 'onboarding_ping' and self._is_bot_pinged(values):
                self.env.user.discussbot_state = "onboarding_attachement"
                self.env.user.discussbot_failed = False
                return _("Yep, I am here! 🎉 <br/>Now, try <b>sending an attachment</b>, like a picture of your cute dog...")
            elif discussbot_state == 'onboarding_attachement' and values.get("attachment_ids"):
                self.env.user.discussbot_state = "idle"
                self.env.user.discussbot_failed = False
            ...
            elif discussbot_state in (False, "idle", "not_initialized") and (_('start the tour') in body.lower()):
                self.env.user.discussbot_state = "onboarding_emoji"
                return _("To start, try to send me an emoji :)")
            # easter eggs
            elif discussbot_state == "idle" and body in ['❤️', _('i love you'), _('love')]:
            ...
            # help message
            elif self._is_help_requested(body) or discussbot_state == 'idle':
                ...
                # repeat question
                if discussbot_state == 'onboarding_emoji':
                    self.env.user.discussbot_failed = True
                    return _("Not exactly. To continue the tour, send an emoji: <b>type</b> <span class=\"o_discussbot_command\">:)</span> and press enter.")
                elif discussbot_state == 'onboarding_attachement':
                    self.env.user.discussbot_failed = True
                    return _("To <b>send an attachment</b>, click on the <i class=\"fa fa-paperclip\" aria-hidden=\"true\"></i> icon and select a file.")
                elif discussbot_state == 'onboarding_command':
                    self.env.user.discussbot_failed = True
                    return _("Not sure what you are doing. Please, type <span class=\"o_discussbot_command\">/</span> and wait for the propositions. Select <span class=\"o_discussbot_command\">help</span> and press enter")
                elif discussbot_state == 'onboarding_ping':
                    self.env.user.discussbot_failed = True
                    return _("Sorry, I am not listening. To get someone's attention, <b>ping him</b>. Write <span class=\"o_discussbot_command\">@OdooBot</span> and select me.")
                return random.choice([
                    _("I'm not smart enough to answer your question.<br/>To follow my guide, ask: <span class=\"o_discussbot_command\">start the tour</span>."),
                    _("Hmmm..."),
                    _("I'm afraid I don't understand. Sorry!"),
                    _("Sorry I'm sleepy. Or not! Maybe I'm just trying to hide my unawareness of human language...<br/>I can show you features if you write: <span class=\"o_discussbot_command\">start the tour</span>.")
                ])
        return False
    ...
    def _is_bot_pinged(self, values):
        discussbot_id = self.env['ir.model.data']._xmlid_to_res_id("base.partner_root")
        return discussbot_id in values.get('partner_ids', [])

    def _is_bot_in_private_channel(self, record):
        discussbot_id = self.env['ir.model.data']._xmlid_to_res_id("base.partner_root")
        if record._name == 'mail.channel' and record.channel_type == 'chat':
            return discussbot_id in record.with_context(active_test=False).channel_partner_ids.ids
        return False
    ...
    def _is_help_requested(self, body):
        ...
        """
        return any(token in body for token in ['help', _('help'), '?']) or self.env.user.discussbot_failed
