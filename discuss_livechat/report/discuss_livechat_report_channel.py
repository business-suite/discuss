...
class ImLivechatReportChannel(models.Model):
    ...
    _name = "discuss_livechat.report.channel"
    ...
    livechat_channel_id = fields.Many2one('discuss_livechat.channel', 'Channel', readonly=True)
    ...
    def init(self):
        ...
        tools.drop_view_if_exists(self.env.cr, 'discuss_livechat_report_channel')
        self.env.cr.execute("""
            CREATE OR REPLACE VIEW discuss_livechat_report_channel AS (
                SELECT
                    C.id as id,
                    C.uuid as uuid,
                    C.id as channel_id,
                ...
                FROM mail_channel C
                    ...
                    JOIN discuss_livechat_channel L ON (L.id = C.livechat_channel_id)
                    LEFT JOIN mail_message MO ON (M.res_id = C.id AND m.model = 'mail.channel' AND MO.author_id = C.livechat_operator_id)
                    LEFT JOIN rating_rating Rate ON (Rate.res_id = C.id and Rate.res_model = 'mail.channel' and Rate.parent_res_model = 'discuss_livechat.channel')
