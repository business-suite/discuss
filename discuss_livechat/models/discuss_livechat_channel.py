...
class ImLivechatChannel(models.Model):
    ...
    _name = 'discuss_livechat.channel'
    ...
    def _default_image(self):
        image_path = modules.get_module_resource('discuss_livechat', 'static/src/img', 'default.png')
    ...
    # relationnal fields
    user_ids = fields.Many2many('res.users', 'discuss_livechat_channel_im_user', 'channel_id', 'user_id', string='Operators', default=_default_user_ids)
    channel_ids = fields.One2many('mail.channel', 'livechat_channel_id', 'Sessions')
    rule_ids = fields.One2many('discuss_livechat.channel.rule', 'channel_id', 'Rules')
    ...
    def _compute_script_external(self):
        view = self.env.ref('discuss_livechat.external_loader')
        values = {
    ...
    def _compute_web_page_link(self):
        for record in self:
            record.web_page = "%s/discuss_livechat/support/%i" % (record.get_base_url(), record.id) if record.id else False
    ...
    def action_view_rating(self):
        ...
        """
        self.ensure_one()
        action = self.env['ir.actions.act_window']._for_xml_id('discuss_livechat.rating_rating_action_livechat')
    ...
    def _open_livechat_mail_channel(self, anonymous_name, previous_operator_id=None, user_id=None, country_id=None):
        """ Return a mail.channel given a livechat channel. It creates one with a connected operator, or return false otherwise
            ...
            If this visitor already had an operator within the last 7 days (information stored with the 'discuss_livechat_previous_operator_pid' cookie),
...
class ImLivechatChannelRule(models.Model):
    ...
    _name = 'discuss_livechat.channel.rule'
    ...
    channel_id = fields.Many2one('discuss_livechat.channel', 'Channel',
        help="The channel of the rule")
    country_ids = fields.Many2many('res.country', 'discuss_livechat_channel_country_rel', 'channel_id', 'country_id', 'Country',
    ...
    def match_rule(self, channel_id, url, country_id=False):
        """ determine if a rule of the given channel matches with the given url
            ...
            :rtype : discuss_livechat.channel.rule
        """
        def _match(rules):
            for rule in rules:
