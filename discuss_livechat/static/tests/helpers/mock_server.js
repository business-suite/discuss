...
MockServer.include({
    ...
    async _performRpc(route, args) {
        if (route === '/discuss_livechat/get_session') {
        ...
        if (route === '/discuss_livechat/notify_typing') {
    ...
    /**
     * Simulates the `/discuss_livechat/get_session` route.
     *
     * @private
    ...
    /**
     * Simulates the `/discuss_livechat/notify_typing` route.
     *
     * @private
     * @param {string} uuid
    ...
    /**
     * Simulates `_get_available_users` on `discuss_livechat.channel`.
     *
     * @private
     * @param {integer} id
     * @returns {Object}
    ...
    _mockImLivechatChannel_getAvailableUsers(id) {
        const livechatChannel = this._getRecords('discuss_livechat.channel', [['id', '=', id]])[0];
        const users = this._getRecords('res.users', [['id', 'in', livechatChannel.user_ids]]);
        return users.filter(user => user.im_status === 'online');
    },
    /**
     * Simulates `_get_livechat_mail_channel_vals` on `discuss_livechat.channel`.
     *
     * @private
     * @param {integer} id
     * @returns {Object}
    ...
    /**
     * Simulates `_get_random_operator` on `discuss_livechat.channel`.
    ...
    /**
     * Simulates `_open_livechat_mail_channel` on `discuss_livechat.channel`.
     *
     * @private
     * @param {integer} id
     * @param {string} anonymous_name
