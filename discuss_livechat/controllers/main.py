...
class LivechatController(http.Controller):
    ...
    @http.route('/discuss_livechat/external_lib.<any(css,js):ext>', type='http', auth='public')
    def livechat_lib(self, ext, **kwargs):
        ...
        bundle = 'discuss_livechat.external_lib'
    ...
    @http.route('/discuss_livechat/load_templates', type='json', auth='none', cors="*")
    def load_templates(self, **kwargs):
        base_url = request.httprequest.base_url
        templates = [
            'discuss_livechat/static/src/legacy/public_livechat.xml',
        ]
        return [tools.file_open(tmpl, 'rb').read() for tmpl in templates]

    @http.route('/discuss_livechat/support/<int:channel_id>', type='http', auth='public')
    def support_page(self, channel_id, **kwargs):
        channel = request.env['discuss_livechat.channel'].sudo().browse(channel_id)
        return request.render('discuss_livechat.support_page', {'channel': channel})

    @http.route('/discuss_livechat/loader/<int:channel_id>', type='http', auth='public')
    def loader(self, channel_id, **kwargs):
        username = kwargs.get("username", _("Visitor"))
        channel = request.env['discuss_livechat.channel'].sudo().browse(channel_id)
        info = channel.get_livechat_info(username=username)
        return request.render('discuss_livechat.loader', {'info': info, 'web_session_required': True}, headers=[('Content-Type', 'application/javascript')])

    @http.route('/discuss_livechat/init', type='json', auth="public", cors="*")
    def livechat_init(self, channel_id):
        available = len(request.env['discuss_livechat.channel'].sudo().browse(channel_id)._get_available_users())
        rule = {}
        if available:
            ...
            matching_rule = request.env['discuss_livechat.channel.rule'].sudo().match_rule(channel_id, url, country_id)
            if matching_rule:
                rule = {
    ...
    @http.route('/discuss_livechat/get_session', type="json", auth='public', cors="*")
    def get_session(self, channel_id, anonymous_name, previous_operator_id=None, **kwargs):
        ...
        return request.env["discuss_livechat.channel"].with_context(lang=False).sudo().browse(channel_id)._open_livechat_mail_channel(anonymous_name, previous_operator_id, user_id, country_id)

    @http.route('/discuss_livechat/feedback', type='json', auth='public', cors="*")
    ...
    @http.route('/discuss_livechat/history', type="json", auth="public", cors="*")
    ...
    @http.route('/discuss_livechat/notify_typing', type='json', auth='public', cors="*")
    ...
    @http.route('/discuss_livechat/email_livechat_transcript', type='json', auth='public', cors="*")
    ...
    @http.route('/discuss_livechat/visitor_leave_session', type='json', auth="public")
